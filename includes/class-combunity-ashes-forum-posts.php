...
class Combunity_Ashes_Forum_Posts {
	...
	public function __construct( $parent ){
		...
		$this->_p->loader->add_filter( 'the_content', $this, 'modify_forum_posts_content', 10, 1);
		$this->_p->loader->add_filter( 'comments_template', $this, 'modify_comments_template', 10, 2);
		$this->_p->loader->add_action( 'comment_post_redirect', $this, 'redirect_to_thank_page', 10, 2);
		$this->_p->loader->add_filter( 'comments_open', $this, 'disable_comments_on_specific_posts', 10, 2 );
	}

	/**
	 * Disable comments on specific post types
	 */
	public function disable_comments_on_specific_posts( $open,$post_id ){
		if (get_post_type($post_id) == 'cforump') {
	    	$open = false;
	  	}
	  	return $open;
	}
	...
	/**
	 * Redirect after a reply is posted
	 */
	public function redirect_to_thank_page( $location, $comment ) {
		
	    if ( 'cpost' === get_post_type( $comment->comment_post_ID ) ){
			if ( $this->_p->notheme() ){
				return '/?combunityipage=view&id='.$comment->comment_post_ID;			
			}
		}
		return $location;
	}
	 ...
	 */
	public function modify_comments_template( $file ){
		if ( 'cpost' === get_post_type() ){
			if ( $this->_p->notheme() ){
				return $this->_p->basedir . '/templates/faketemplate.php';				
			}
		}else if ( 'cforump' === get_post_type() ){
			return 'bur';
		}
		return $file;
	}
	...
	/**
	 * Modify forum post content so that it loads in an iframe
	 */
	public function modify_forum_posts_content( $content ){
		if ( 'cpost' !== get_post_type() ){
			return $content;
		}
		if ( !is_single() ){
		...
		if ( $this->_p->notheme() ){
			return $this->_p->Frontend->build_cpost(array('content'=>$content));
		}else{
			return $content;
		}
	}
