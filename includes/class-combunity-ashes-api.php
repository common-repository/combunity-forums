...
class Combunity_Ashes_API {
	...
	public function get_user_link( $id ){
		if ( $this->_p->notheme() ){
			$user_link = $this->_p->Pages->getlink('user');
			return add_query_arg( 'uid', $id, $user_link );
			// return $user_link;
		}else{
	...
	/**
	 * Returns if combunity is running in notheme mode
	 *
	 *
	 * @since    1.0.0
	 * @access   public
	...
	public function get_edit_thread_link(){
		if ( Combunity_Api()->can_user_do("delete", get_the_ID() ) ){
			$link = '#';
			// var_dump( $this->_p->Pages );
			$link = $this->_p->Pages->getlink('edit');
			$text = __('Edit');
			$id = get_the_ID();
			$ahref = sprintf( '<a href="%1$s" class="combunity-edit-thread-link" noblank="true" data-id="%3$s">%2$s</a>', $link, $text, $id );
			return $ahref;
		}
	}
	...
	public function get_delete_thread_link(){
		if ( Combunity_Api()->can_user_do("delete", get_the_ID() ) ){
			...
			$text = __('Delete');
			$id = get_the_ID();
			$ahref = sprintf( '<a href="%1$s" class="combunity-delete-thread-link" noblank="true" data-id="%3$s">%2$s</a>', $link, $text, $id );
			return $ahref;
		}
	}
	...
	public function getpublicdirpath() {
		$url = dirname(plugin_dir_url( $this->_p->file ));
		return $url;
    }
	...
	public function gettemplatedirpath() {
		return $this->_p->basedir . '/templates/';
    }

    /**
     * Get internal page link 
     */
    public function getipagelink( $name ){
    	return  $this->_p->Pages->getlink( $name );
    	// return $this->Pages->getlink( $name );
    }
